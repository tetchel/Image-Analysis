...
enum Mode { WINDOW, SCANLINE, GLOBAL };
enum SCANLINE_MODE {ABSOLUTE, QUADRATIC, TRUNCATED};
...
public:
	...
	inline const Table2D<int>& getLabeling() const { return m_labeling; }
	...
	int stereo(Mode mode, Table2D<RGB> image, Table2D<RGB> image_R, int w, int l, SCANLINE_MODE, void(*draw_function));
...
private:
	...
	Table2D<int> m_labeling; // labeling of image pixels
